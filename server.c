...
int main(int argc, char *argv[])
{
    ...
    while(1) {
        ...
        */
        ...
        int total_packet = (sourceLength + (MAX_PAYLOAD_SIZE-1)) / MAX_PAYLOAD_SIZE;
        response_packet.max_num = total_packet;
        response_packet.seq_num = 0;
        int start_of_seq = 0;
        int current_pkt = 0;
        int end_of_seq;
        if (window_size > total_packet)
            end_of_seq = total_packet;
        else
        ...
        int* ACK_table = (int*) malloc(window_size*sizeof(int));
        int i;
        ...
        while(1){
            ...
            if (ACK_table[current_pkt - start_of_seq] == -2 && !last){
                ...
                response_packet.seq_num = seq_num;
                response_packet.max_num = total_packet;
                ...
                ACK_table[current_pkt - start_of_seq] = time_diff;
                ...
                if (response_packet.type == 3)
                    ...
                    status = "FIN";
                printf("Sending packet %d %d %s\n", response_packet.seq_num, cwnd, status);
            }
            ...
            for (i = start_of_seq; i<end_of_seq; i++){
                if (ACK_table[i-start_of_seq] > 0){
                    ...
                    if (time_diff - ACK_table[i-start_of_seq] >= MAX_RETRANS_TIME){
                        // printf("(%d)\n", time_diff - ACK_table[i-start_of_seq]);
                        if ((packet_window[i - start_of_seq]).seq_num
                            == (packet_window[i - start_of_seq]).max_num)
                        ...
                        printf("Sending packet %d %d Retransmission\n", packet_window[i - start_of_seq].seq_num, cwnd);
                        ACK_table[i-start_of_seq] = diff_ms(end, start);
                    }
                }
            }
            ...
            if(ack.type == 2) {
                int pkt = (ack.seq_num + ack.seq_count*30720) / MAX_PAYLOAD_SIZE;
            ...
            if (last) {
                ...
                if (finished == 0){
                    ...
                    while (1) {
                        ...
                        fin_packet.seq_num = ack.seq_num + response_packet.data_size + HEADER_SIZE;
